#!/usr/bin/env bash
# L1 joint GRPO on stratified k2 decision states (30 tasks × 2 variations).
set -euo pipefail
cd "$(dirname "$0")/.."
...
LOG="${LOG:-artifacts/l1/l1_joint_stratified_k2_train.log}"

for path in \
  "$STATES" \
  "artifacts/checkpoints/action_id_sft_smoke/gold_contract/sub_agent/best/adapter_model.safetensors"; do
  [[ -f "$path" ]] || { echo "[l1-joint-k2] missing: $path (run scripts/l1_collect_states_stratified_k2.sh first)" >&2; exit 1; }
done
...
echo "[l1-joint-k2] GPU=$GPU config=$CONFIG states=$STATES start_iter=$START_ITER"
echo "===== L1 joint stratified k2 $(date -Iseconds) =====" | tee -a "$LOG"
