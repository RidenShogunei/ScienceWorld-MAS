"""Fixed, reproducible ScienceWorld evaluation episode lists."""
...
def generate_stratified_episodes(
    ...
    """Sample K fixed variations per task type (stratified, seed-controlled)."""
    if k_per_task <= 0:
    ...
    for task_name in sorted(names):
        ...
        take = min(k_per_task, len(variations))
        for variation_id in variations[:take]:
