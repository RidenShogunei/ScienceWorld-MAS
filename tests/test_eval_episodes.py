"""Tests for fixed stratified evaluation episode lists."""
...
def test_generate_stratified_episodes_is_reproducible_and_balanced(tmp_path):
    runner = FakeRunner(
    ...
    assert all(spec.split == "dev" for spec in first)


def test_save_and_load_episode_list_roundtrip(tmp_path):
    specs = [
    ...
    assert payload["metadata"]["protocol"] == "stratified_k_per_task"
